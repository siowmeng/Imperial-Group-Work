...
```{r echo = FALSE, message = FALSE}
library(plyr)
directorcount <- count(movies, 'director_name')
sorteddirectorcount <- arrange(directorcount,desc(freq))
topdirectorcount <- sorteddirectorcount[1:20,]
topdirectorcount$director_name <- factor(topdirectorcount$director_name, levels = topdirectorcount$director_name[order(topdirectorcount$freq)])

ggplot(topdirectorcount, aes(x=director_name, y=freq, fill=director_name)) + geom_bar(stat="identity") + coord_flip() + guides(fill=FALSE) + scale_y_continuous(expand = c(0, 0)) + labs(x = "", y="Number of movies in sample") + ggtitle("Top 20 directors with multiple IMDB top 5000 movies") +  scale_y_continuous(expand = c(0,0)) +  expand_limits(y = c(0,1.05 * max(topdirectorcount$freq)))
