...
ggplot(head, aes(x=director_name, y=freq, fill=director_name)) + geom_bar(stat="identity") + coord_flip() + guides(fill=FALSE) + scale_y_continuous(expand = c(0, 0)) + labs(x = "", y="Frequency") + ggtitle("Top 20 directors with multiple IMDB top 5000 movies") +  scale_y_continuous(expand = c(0,0)) +  expand_limits(y = c(0,1.05 * max(head$freq)))
...
## doesn't work
replaceNAwithmean <- function(columnname) {
 k <- which(colnames(movies)==columnname)
 print(k)
 print(round(mean(movies[[k]], na.rm=TRUE),0))
 print(movies[k][is.na(movies[k]==TRUE)])
 movies[k][is.na(movies[k]==TRUE)] <- round(mean(movies[[k]], na.rm=TRUE),0)
 print(movies[k][is.na(movies[k]==TRUE)])}
replaceNAwithmean("duration")
