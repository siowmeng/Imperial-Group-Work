...
ggplot(movies, aes(title_year)) + 
  ...
  theme(axis.ticks.y = element_blank()) + 
  scale_x_continuous(breaks = seq(1920, 2016, by = 4))
  theme_few()
```
...
ggplot(movies, aes(x = budget, y = gross, colour = profitable)) +
  geom_jitter(alpha = 0.2, width = 0.05) + 
  scale_x_log10(breaks = c(1e+02, 1e+04, 1e+06, 1e+08), 
                labels = c("100", "10,000", "1 million", "100 millions")) +
  scale_y_log10(breaks = c(1e+02, 1e+04, 1e+06, 1e+08), 
                labels = c("100", "10,000", "1 million", "100 millions")) +
...
movies_and_genres3 <- movies_and_genres2[movies_and_genres2$value == 1, ] 

ggplot(movies_and_genres3, aes(x = profit, y = score)) + 
  geom_jitter(alpha = 0.1) +
  scale_x_log10(breaks = c(1e+02, 1e+04, 1e+06, 1e+08, 1e+10), 
                labels = c("100", "10,000", "1 million", "100 millions", "1 billion")) +
  labs(x = "Profit", y = "IMDb Score") +
  facet_wrap(~ variable, nrow = 3, ncol = 8) +
  theme_few() 


```

Now, let's see the most popular keywords.
...
top_actors <- actors_full %>% arrange(desc(fb_likes)) %>% top_n(20, fb_likes)

ggplot(top_actors, aes(x = movies_played, y = fb_likes)) + 
  geom_point(size = 1.5) + 
  geom_text_repel(aes(label = name), size = 3) +
  labs(title = "Most Famous Actors", x = "Movies played", y = "Facebook likes") +
  scale_y_log10(breaks = c(25e+03, 5e+04,1e+05, 2e+05, 4e+05), labels = c("25,000", "50,000", "100,000", "200,000", "400,000")) +
  theme_few()
```

We will now focus on the most profitable movies:

```{r}
movies_prof <- movies %>% top_n(100, profit)
# All of them are profitable and are in English
movies_prof$profitable <- NULL
movies_prof$language <- NULL
