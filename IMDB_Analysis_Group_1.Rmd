...
counted <- count(movies, countries)
sorted <- arrange(counted, desc(n))
topcountries <- sorted[1:10,]
topcountries$country <- factor(topcountries$country, levels = topcountries$country[order(topcountries$n)]
...
*I have changed head but it is still not knitting for me either, so I will see what's wrong - Nikhita*
...
# ggplot(Chi, aes(x = gross, y = imdb_score)) + geom_point()
# ggplot(Ind, aes(x = gross, y = imdb_score)) + geom_point()
>>>>>>> 7081d8198783a25e75d18472dd5e20800384e0e8
