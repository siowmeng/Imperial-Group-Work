...
There are `r nrow(sorteddirectorsummary)` distinct directors in the data sample. The top 20 directors with the highest number of movies in our sample are as below:
...
The density of movies per director is as below:
...
ggplot(movieswithdirectordata, aes(x = dir_number_of_movies, y = gross, colour = decade)) + 
  geom_jitter(alpha = 0.1) + scale_y_continuous(label=human_usd) + labs(title = "Gross Revenue and # of director movies for different decades", x = "# of Movies by Director", y = "Gross Revenue") + facet_wrap(~ decade, ncol = 5) + theme(legend.position = "None")
```

And by genres, then we see the below:

```{r eval=FALSE, echo=FALSE}
##this doesn't work at the moment when knitting, but does when running only George's chunks, so need to see what's going on here.
##data prep to get genre + director data together
##note: would be good if we could just use the same data frame
movies$genres <- strsplit(movies$genres, "[|]")
genres_idx <- movies[, c("movie_title", "genres")]
i = 1
mat <- matrix(rep(0, (dim(movies)[1] * length(genres))), nrow = dim(movies)[1])
...
colnames(mat) <- genres
movies_and_genres <- data.frame(mat)
movies_and_genres <- cbind(gross = movies$gross, score = movies$imdb_score, director_name = movies$director_name, movies_and_genres, stringsAsFactors = FALSE)
movies_and_genres <- melt(movies_and_genres, id = c("gross", "score", "director_name"))
movies_and_genres$variable <- gsub("[.]", " ", movies_and_genres$variable)
movies_and_genres <- movies_and_genres[movies_and_genres$value == 1, ] 
movies_and_genres$value <- NULL
colnames(movies_and_genres) <- c("gross", "score", "director_name","genre")
movies_and_genres$genre <- factor(movies_and_genres$genre, levels = genres_sum$genre)
movies_and_genres <- movies_and_genres[complete.cases(movies_and_genres), ]

movies_and_genres_withdirectordata <- merge(movies_and_genres, sorteddirectorsummary, by="director_name")
```

```{r eval=FALSE, echo=FALSE}
##requires the above
ggplot(movies_and_genres_withdirectordata, aes(x = number_of_movies, y = gross, colour = genre)) + 
  geom_jitter(alpha = 0.1) + scale_y_continuous(label=human_usd) + labs(title = "Gross Revenue and # of director movies for different genres", x = "# of Movies by Director", y = "Gross Revenue") + facet_wrap(~ genre) + theme(legend.position = "None")
```

Next steps:
- look at the graph without the outliers (top 20) - I think there will be an upwards trend here
...
movies_and_genres_full <- data.frame(movies_and_genres, predicted_gross = genres_model_gross$fitted)
...
movies_and_genres_full <- data.frame(movies_and_genres, predicted_score = genres_model_score$fitted)
