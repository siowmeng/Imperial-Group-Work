...
ggplot(data = budgetcountry) + geom_boxplot(aes(x = country, y = budget, fill = country)) + scale_y_log10() + stat_summary(fun.y=mean, colour="red", geom="point", size=1, show.legend = FALSE) + labs(x = "Country", y="Budget (USD)") + ggtitle("Budget of films by country") + theme(legend.position="none")
...
model2b <- lm(gross ~ budget + movie_facebook_likes + cast_total_facebook_likes + director_facebook_likes + imdb_score + country + content_rating + num_user_for_reviews + num_voted_users, data = train)
summary(model2b)
...
model4b <- lm(gross ~ budget + movie_facebook_likes + cast_total_facebook_likes + director_facebook_likes + imdb_score + country + content_rating, data = train[train$title_year >= 2010, ])
summary(model4b)
