...
The below table shows the number of missing values found in each column:

*LF comment - can we make this smaller/into more rows so that it takes up less page space?*
...
In this section, we look at whether the country of origin has a huge influence on the number of movies and the revenue that a movie generates. Then, we check if there is any relationship between the gross revenue and IMDB score for countries with more than 10 movies.
...
In terms of frequency, we observe that the number of movies originated in USA is almost 10 times that of UK. Also, the top five countries with the most number of movies are in Europe. On the other end, Asian countries have no more than 20 movies in our dataset. 

Next, we will look at the gross revenues for the top 12 countries. More than 50% of US, UK and Germany releases are generating high revenues of over 10 million. On the other hand, more than 25% of releases in China and Spain are generating revenues of lower than 1 million.
...
In order to get a better understanding of gross revenues they can be studied over time. This analysis will determine if title year has a significant effect on gross revenues and if there were any excpetionally successful years for gross revenues.
...
ggplot(temp, aes(x = factor(title_year), y = gross, fill = factor(title_year))) + 
    ...
    labs(title = "Gross revenues against year", x = "", y = "Gross Revenue") + 
    scale_y_log10(breaks = c(1, 1e+02, 1e+04, 1e+06, 1e+08, 1e+10), 
...
The above plot shows gross revenues against title years for all movies. It shows that there might be a slight upward trend in gross revenues. Years prior to 1972 show a higher variance than those after 1972. In addition, movies with a release date prior to 1972 have lower mean and median gross revenues. The exception is one outlier with a release date in 1937. Several release years, however, include fewer than ten movies and can therefore not be considered representative of a specific year's gross revenue. To get a more accurate picture of the development of gross revenues over time the following two plots will remove years with less than ten entries.
...
The two plots above plot title years with more than ten entries. This gives a less biased represenation of the development of gross revenues. From the first plot, revenues now appear more constant. Recent years have more outliers with a lower gross revenue which suggests a downward trend. The slighlty downward sloping trendline on the second plot confirms this assumption. (conclude? or conclude at the end?)
...
pander(t.test(movies$gross[movies$language == "English"], movies$gross[movies$language != "English"], alternative = "greater"))

pander(var.test(movies$gross[movies$language == "English"], movies$gross[movies$language != "English"], alternative = "greater"))
```
...
pander(t.test(movies$gross[movies$country == "USA"], movies$gross[movies$country != "USA"], alternative = "greater"))

pander(var.test(movies$gross[movies$country == "USA"], movies$gross[movies$country != "USA"], alternative = "greater"))
```
...
pander(t.test(movies$gross[movies$content_rating == "R"], movies$gross[movies$content_rating == "PG-13"]))

pander(t.test(movies$gross[movies$content_rating == "PG-13"], movies$gross[movies$content_rating == "PG"]))

pander(t.test(movies$gross[movies$content_rating == "R"], movies$gross[movies$content_rating == "PG"]))
```
...
#correlation test
pander(cor.test(movies$budget, movies$imdb_score))
```
...
#correlation test
pander(cor.test(movies$budget, movies$gross))
```
...
#correlation test
pander(cor.test(movieswprofit$budget, movieswprofit$grossprofitmargin, use = "complete.obs"))
```
...
pander(table(movieswprofit$profitable))
```
...
With Hollywood, North America arguably has the most successful movie industry in the world. Monetary success, however, does not necessarily imply good quality. This tradeoff is obvious in many multinational companies that produce large volumes albeit at lower qualities. If IMDB scores are taken as proxy for movie quality, Europe's movie industry can be compared to North America's. To test the claim whether there is a difference in quality between North American and European movies a t-test can be conducted. 

This section tests whether North American movies have a higher mean IMDB score than European ones. Intuitively, one would assume that North American movies score higher than European ones. This intuition stems from the prevalence of North American movies in media. 
...
pander(t.test(Europe$imdb_score, North_America$imdb_score))
...
pander(cor.test(movieswithdirectordata$imdb_score, movieswithdirectordata$dir_number_of_movies))
...
pander(cor.test(movieswithdirectordata$gross, movieswithdirectordata$dir_number_of_movies))
...
ggplot(movieswithdirectordata, aes(x = factor(more_than_ten_movies), y = gross, fill = factor(more_than_ten_movies))) + 
    ...
    scale_x_discrete(labels = c("Fewer than 10", "More than 10"))
```
...
*Can't format the 2 following tests, otherwise my knitting i halted :( --George*
...
stargazer(list(fullmodel), type = "html", align = TRUE, dep.var.labels = "Gross revenue", no.space = TRUE, title = "Linear Regression results")
```
</center>
...
stargazer(list(fbmodel, fbmodel2010), type = "html", align = TRUE, column.labels=c("For all years", "For 2010 onwards"),
...
stargazer(list(knownvariablesmodel, knownvariablesmodel2010), type = "html", align = TRUE, dep.var.labels = "Gross revenue", no.space = TRUE, title = "Linear Regression results")
```
</center>
...
stargazer(pre2010model, type = "html", align = TRUE, dep.var.labels = "Gross revenue", no.space = TRUE, title = "Linear Regression results")
```
</center>
...
##calculate the prediction and MSE for all 3 models using all data and data post 2010
fullmodelpre <- predict(fullmodel, test)
fullmodelmse <- sqrt(mean((fullmodelpre - test$gross)^2))
fullmodel2010pre <- predict(fullmodel2010, test[test$title_year >= 2010, ])
fullmodel2010mse <- sqrt(mean((fullmodel2010pre - test$gross[test$title_year >= 2010])^2))
fbmodelpre <- predict(fbmodel, test)
fbmodelmse <- sqrt(mean((fbmodelpre - test$gross)^2))
fbmodel2010pre <- predict(fbmodel2010, test[test$title_year >= 2010, ])
fbmodel2010mse <- sqrt(mean((fbmodel2010pre - test$gross[test$title_year >= 2010])^2))
knownvariablesmodelpre <- predict(knownvariablesmodel, test)
knownvariablesmodelmse <- sqrt(mean((knownvariablesmodelpre - test$gross)^2))
knownvariablesmodel2010pre <- predict(knownvariablesmodel2010, test[test$title_year >= 2010, ])
knownvariablesmodel2010mse <- sqrt(mean((knownvariablesmodel2010pre - test$gross[test$title_year >= 2010])^2))

##turn MSE data into a table for display with columns of "all data" and "post 2010", and rows of model type
msetable <- data.frame(alldata=c(fullmodelmse, fbmodelmse, knownvariablesmodelmse), post2010 = c(fullmodel2010mse, fbmodel2010mse, knownvariablesmodel2010mse))
rownames(msetable) <- c("Full Model", "Facebook Model", "Known Variables Model")
colnames(msetable) <- c("Model using all data", "Model using post 2010 data")
pander(msetable, format="markdown")
```
</center>
...
dffbmodel <- data.frame(y_actual = test$gross, y_predict = fbmodelpre)
fbmodelplot1 <- ggplot(data = dffbmodel, aes(x = y_predict, y = y_actual)) + geom_point(colour = "red") + 
    geom_abline(colour = "blue") + xlab("Predicted Gross Revenue (US$)") + 
    ylab("Actual Gross Revenue(US$)") + 
    coord_cartesian(xlim = c(-0.2e8, 1.5e8), ylim = c(0, 2.5e8)) + 
    scale_y_continuous(label = human_usd)  + scale_x_continuous(label = human_usd) + ggtitle("Facebook Model All Data")
fbmodelplot2 <-ggplot(data = dffbmodel, aes(x = y_predict, y = y_actual - y_predict)) + 
    geom_point(colour = "red") + geom_hline(yintercept = 0, colour = "blue") + 
    xlab("Predicted Gross Revenue (US$)") + ylab("Residual(US$)") + 
    coord_cartesian(xlim = c(-0.2e8, 2e8), ylim = c(-1.5e8, 1.5e8)) + 
    scale_y_continuous(label = human_usd) + scale_x_continuous(label = human_usd) + ggtitle("Facebook Model All Data")
##Plot the predictions and residuals for facebook model post 2010
dffbmodel2010 <- data.frame(y_actual = test$gross[test$title_year>=2010], y_predict = fbmodel2010pre)
fbmodelplot3 <- ggplot(data = dffbmodel2010, aes(x = y_predict, y = y_actual)) + geom_point(colour = "blue") + 
    geom_abline(colour = "red") + xlab("Predicted Gross Revenue (US$)") + 
    ylab("Actual Gross Revenue(US$)") + 
    coord_cartesian(xlim = c(-0.2e8, 1.5e8), ylim = c(0, 2.5e8)) + 
    scale_y_continuous(label = human_usd)  + scale_x_continuous(label = human_usd) + ggtitle("Facebook Model post 2010")
fbmodelplot4 <-ggplot(data = dffbmodel2010, aes(x = y_predict, y = y_actual - y_predict)) + 
    geom_point(colour = "blue") + geom_hline(yintercept = 0, colour = "red") + 
    xlab("Predicted Gross Revenue (US$)") + ylab("Residual(US$)") + 
    coord_cartesian(xlim = c(-0.2e8, 2e8), ylim = c(-1.5e8, 1.5e8)) + 
    scale_y_continuous(label = human_usd) + scale_x_continuous(label = human_usd) + ggtitle("Facebook Model post 2010")
grid.arrange(fbmodelplot1,fbmodelplot2,fbmodelplot3,fbmodelplot4, ncol=2)
```
...
<center>
```{r echo=FALSE, results="asis"}
modelperf2010 <- data.frame(matrix(data = c(rmseFB2010, rmseFull2010, rmseKnown2010), nrow = 3))
...
pander(modelperf2010, type = "html")
```
</center>
