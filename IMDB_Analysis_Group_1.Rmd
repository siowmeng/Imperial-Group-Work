...
- Aside from budget, content total cast facebook likes, director facebook likes, ratings duration and key genres are also strong factors in determining whether a movie performs well in the box office or not
...
However, there are many limitations to this dataset, including a limited number of variables and a clear bias in the number of English movies vs other language movies. Thus, the conclusions drawn from this analysis need to be verified with a more extensive data set. Moving forward, it would be very useful to obtain data from other movie databases indigenious to countries other than the US and the UK, e.g. [Indian Bollywood movies](http://www.bollywoodmdb.com). Though our model is able to predict revenues considerably well for a first attempt, and specifically for movies with revenue up to 50 million USD, it still has difficulties estimating the revenue with decent accuracy for movies with even higher gross revenues. Acquiring more data points and having datasets with relevant variables different from the current ones can help to improve the accuracy of our predictive models.
...
movies <- movies[complete.cases(movies[c("budget","gross")]),]
...
This cleansed dataset (of `r nrow(movies)` rows) contains no missing values and is used for the remainder of the analysis. The head of the data frame is printed here:
...
The following chart illustrates the changes of average gross revenues from 1920 to 2016. We can observe a large fluctuation between 1920 and 1968, this can be attributed to the lack of data during this period. The dataset only contains `r nrow(movies[movies$title_year <= 1968, ])` records from 1920 to 1968, whereas there are `r nrow(movies[movies$title_year > 1968, ])` records of movies screened after 1968.
...
ggplot(data = revYear, aes(x = title_year, y = meanRev)) + 
    ...
    scale_y_log10(breaks = c(1, 1e+02, 1e+04, 1e+06, 1e+08, 1e+10),
                  labels = c("1", "100", "10,000", "1 million", "100 million", "10 billion"), limits = c(1, 1e+10)) +
...
For non-English movies, we can observe fluctuations across the entire period (from 1920 to 2016). This is likely caused by the relative lack of non-English movie data. There are only `r nrow(movies[movies$language != "English", ])` non-English movies in the dataset, compared to `r nrow(movies[movies$language == "English", ])` English movies. In recent years (after year 1990), the average gross revenue of non-English movies range from below US$10 thousand to over US$100 million.
...
ggplot(countries, aes(country, gross, fill = country)) + 
    ...
    theme_bw() + 
    theme(legend.position = "None") + theme(axis.ticks.y = element_blank(), panel.grid.major.y = element_blank(), axis.title.x = element_blank()) +
    scale_y_log10(breaks = c(1, 1e+02, 1e+04, 1e+06, 1e+08, 1e+10), 
                  labels = c("1", "100", "10,000", "1 million", "100 million", "10 billion"), limits = c(1, 1e+10)) +
    annotation_logticks(sides = "lr", colour = "gray") 
```
...
ggplot(countries, aes(x = score, y = gross, colour = country)) + 
  geom_jitter() + scale_x_log10() + labs(title = "IMDB Score and Gross Revenue by Country", x = "IMDB Score", y = "Gross Revenue") +
  facet_wrap(~ country) +
  theme_few() + 
  theme(legend.position = "None") + 
    scale_y_log10(breaks = c(1e+02, 1e+04, 1e+06, 1e+08, 1e+10), 
                  labels = c("100", "10,000", "1 million", "100 million", "1 billion")) +
    geom_jitter(alpha = 0.2) + 
    labs(title = "IMDB Score and Gross Revenue by Country", x = "IMDB Score", y = "Gross Revenue") +
...
ggplot(data = movies, aes(x = budget)) + 
    ...
    scale_x_log10(breaks = c(1, 1e+02, 1e+04, 1e+06, 1e+08, 1e+10), 
                  labels = c("1", "100", "10,000", "1 million", "100 million", "10 billion"), limits = c(1, 1e+10)) + 
    labs(x = "Budget", y = "Count", title = "Budget of Movies") +
...
ggplot(temp, aes(x = factor(title_year), y = gross, fill = factor(title_year))) + 
    ...
    labs(title = "Gross Revenue from 1920 to 2016", x = "", y = "Gross Revenue") + 
    scale_y_log10(breaks = c(1, 1e+02, 1e+04, 1e+06, 1e+08, 1e+10), 
                  labels = c("1", "100", "10,000", "1 million", "100 million", "10 billion"), limits = c(1, 1e+10)) +
...
# table(movies$title_year) command shows that from 1980 onwards all the years have at least ten entries
...
p1 <- ggplot(temp, aes(x = factor(title_year), y = gross, fill = factor(title_year))) + 
    ...
    theme_bw() + 
    theme(axis.text.x = element_text(angle = 45, hjust = 0.5, vjust = 0.5), legend.position = "None", axis.title.x = element_blank()) +
    labs(title = "Gross Revenue for Years with more than 10 Movies", x = "", y = "Gross Revenue") +
    scale_y_log10(breaks = c(1, 1e+02, 1e+04, 1e+06, 1e+08, 1e+10), 
                  labels = c("1", "100", "10,000", "1 million", "100 million", "1 billion"),  limits = c(1, 1e+10)) +
   annotation_logticks(sides = "lr", colour = "gray") 
...
# subset movies data frame to include all movies from 1980 onwards
temp <- movies[movies$title_year >= 2010, ]
# plot the range of gross revenues for all years
p1 <- ggplot(temp, aes(x = factor(title_year), y = gross, fill = factor(title_year))) + 
    geom_boxplot() +
    theme_bw() + 
    theme(axis.text.x = element_text(angle = 45, hjust = 0.5, vjust = 0.5), legend.position = "None", axis.title.x = element_blank()) +
    labs(title = "Gross Revenue from 2005 to 2016", x = "", y = "Gross Revenue") +
    scale_y_log10(breaks = c(1, 1e+02, 1e+04, 1e+06, 1e+08, 1e+10), 
                  labels = c("1", "100", "10,000", "1 million", "100 million", "1 billion"),  limits = c(1, 1e+10)) +
   annotation_logticks(sides = "lr", colour = "gray") 
...
The above plot shows the development of gross revenue since the widespread popularity of Facebook in 2010. There is an upward trend (note, the y axis is logarithmic), which is a different pattern compared to the plot with displaying all years with more than ten entries.
...
Before looking into the relationship with the key output variable, gross revenue, we look into some descriptive statistics of the director data in the sample. There are `r nrow(sorteddirectorsummary)` distinct directors represented, the density of movies per director is shown on the figure on the right. This is clearly positively skewed, with many directors having only one movie in the dataset, and only a few having significantly more. In fact, approximately  `r dirpercentage`% of those directors have only one movie in this sample, leaving `r 100-dirpercentage`% that have more than one.
...
ggplot(movieswithdirectordata, aes(x = dir_number_of_movies, y = gross)) + 
    ...
    theme_bw() +
    labs(x = "Number of movies\nper director", y = "Gross Revenue") + scale_y_log10(breaks = c(1, 1e+02, 1e+04, 1e+06, 1e+08, 1e+10), 
                  labels = c("1", "100", "10,000", "1 million", "100 million", "10 billion"), limits = c(1, 1e+10)) +
    annotation_logticks(sides = "lr", colour = "gray") + 
    scale_x_continuous(breaks = c(0, 5, 10, 15, 20, 25, 30), limits = c(0, 30)) +
    geom_smooth()
...
pander(c, caption=paste("Pearson's Product-Moment Correlation test for number of movies per director against budget, correlation =",round(cor(movieswithdirectordata$dir_number_of_movies, movieswithdirectordata$budget),3)))
```

Even though this correlation was found to be statistically significant, with a p-value of `r c$p.value`, it is quite small (`r round(cor(movieswithdirectordata$dir_number_of_movies, movieswithdirectordata$budget), 3)`), so analysis of movies by director sample on gross revenue can be continued without an obvious confounder of budget.
...
ggplot(movies_and_genres_withdirectordata, aes(x = dir_number_of_movies, y = gross, colour = genre)) + 
    ...
    scale_y_log10(breaks = c(1, 1e+02, 1e+04, 1e+06, 1e+08, 1e+10), 
                  labels = c("1", "100", "10,000", "1m", "100 m", "10 bn"), limits = c(1, 1e+10)) +
    theme_bw() +
    annotation_logticks(sides = "lr", colour = "gray") + 
...
In terms of the impact that this pattern may have on our dataset, only approximately `r morethantendirpercentage`% directors have more than ten movies, which, when considering movies, constitutes approximately `r morethantenmovpercentage`% of movies in our sample, so the positive trend at the low end of the scale may be more useful in prediction than the behaviour at the higher end.
...
```{r englishtest, echo = FALSE, results = TRUE, fig.width = 6, fig.align = "center"}
...
ggplot(data = movies, aes(x = english, y = gross, fill = english)) + 
    ...
    scale_y_continuous(breaks = c(0, 15e+06, 30e+06, 45e+06, 60e+06, 75e+06), 
                       labels = c("0", "15 million", "30 million", "45 million", 
...
pander(t.test(movies$gross[movies$language == "English"], movies$gross[movies$language != "English"], alternative = "greater"), caption = "T-Test of means - Gross Revenue for English and Non-English language movies")
```
...
pander(var.test(movies$gross[movies$language == "English"], movies$gross[movies$language != "English"], alternative = "greater"), caption = "F-Test of variance - Gross Revenue for English and Non-English language movies")
```
...
```{r usatest, echo = FALSE, results = TRUE, fig.width = 6, fig.align = "center"}
...
pander(t.test(movies$gross[movies$country == "USA"], movies$gross[movies$country != "USA"], alternative = "greater"), caption="T-Test of means - Gross Revenue for USA and Non-USA movies")
```
...
pander(var.test(movies$gross[movies$country == "USA"], movies$gross[movies$country != "USA"], alternative = "greater"), caption="F-Test of variance - Gross Revenue for USA and Non-USA movies")
```
...
pander(t.test(movies$gross[movies$content_rating == "R"], movies$gross[movies$content_rating == "PG-13"]), caption="R vs PG-13")

pander(t.test(movies$gross[movies$content_rating == "PG-13"], movies$gross[movies$content_rating == "PG"]), caption="PG-13 vs PG")

pander(t.test(movies$gross[movies$content_rating == "R"], movies$gross[movies$content_rating == "PG"]), caption="R vs PG")
```
...
#correlation test
pander(cor.test(movies$budget, movies$gross), caption=paste("Pearson's Product-Moment Correlation Test for Gross Revenue against Budget, correlation =",round(cor(movies$budget, movies$gross),3)))
```

The covariance value of `r round(cov(movies$budget, movies$gross), 3)` suggests that there is an upward trend - as budget increases, revenue from the movie increases. The Pearson's correlation test between budget and gross gives us a value of 0.22, which suggests that there is a positive correlation between the budget of a movie and its gross revenue, but the correlation itself is not very strong. Under the confidence level of 95%, the p-value is significant (5.56e-44) and hence, we can reject the null hypothesis that the true correlation is not equal to 0.
...
ggplot(budgetcountry, aes(x = budget, y = gross, colour = country)) + 
    geom_jitter(alpha = 0.2) +  
    labs(title = "Budget and Gross Revenue by Country", x = "Budget", y = "Gross Revenue") +       
    theme_bw() + 
    scale_x_log10(breaks = c(1, 1e+02, 1e+04, 1e+06, 1e+08, 1e+10), 
                  labels = c("1", "100", "10,000", "1m", "100m", "10bn"), limits = c(1, 1e+10)) +
    scale_y_log10(breaks = c(1, 1e+02, 1e+04, 1e+06, 1e+08, 1e+10), 
                  labels = c("1", "100", "10,000", "1 million", "100 million", "10 billion"), limits = c(1, 1e+10)) +
    annotation_logticks(sides = "tlbr", colour = "gray") +
    facet_wrap(~ country) +
...
We will now see in more detail how the gross revenue of movies differs based on their genre. Because the genre is a nominal variable, the predicted gross revenue for the movies of each genre will equal to the the average value of gross revenue of the observed movies of that particular genre. In the following diagram we can see which genres have the highest and lowest gross revenue on average and in what order.
...
ggplot() + 
    ...
    scale_x_log10(breaks = c(0.15e+08, 0.5e+08, 1e+08), 
                  labels = c("15 million", "50 million", "100 million")) +
...
We see that Documentaries are the less profitable by far with an average estimated gross revenue of US$ `r round(min(avg_gross$predicted_gross)/10^6, 2)` million, while the most profitable ones appear to be Animation movies, with a gross revenue of US$ `r round(max(avg_gross$predicted_gross)/10^6, 2)` million, followed by Adventure, Family, Fantasy and Action movies. The difference between the highest and lowest average gross revenues based on genres is US$ `r round((max(avg_gross$predicted_gross) - min(avg_gross$predicted_gross))/10^6, 2)` million. The rest genres appear to have more or less the same predicted gross revenue. To find out if those differences in the predicted  gross revenue values are significant, we will perform some T-tests between the average gross revenues of movies of different genres, which are actually corresponging to the predicted gross revenue values.

```{r echo = FALSE, results="asis"}
pander(t.test(movies_and_genres_full$gross[movies_and_genres_full$genre == "Animation"], movies_and_genres_full$gross[movies_and_genres_full$genre == "Documentary"]), caption = "T-Test for Gross Revenue between Animation and Documentary movies")

pander(t.test(movies_and_genres_full$gross[movies_and_genres_full$genre == "Animation"], movies_and_genres_full$gross[movies_and_genres_full$genre == "Action"]), caption = "T-Test for Gross Revenue between Animation and Action movies")

pander(t.test(movies_and_genres_full$gross[movies_and_genres_full$genre == "Animation"], movies_and_genres_full$gross[movies_and_genres_full$genre == "Adventure"]), caption = "T-Test for Gross Revenue between Animation and Adventure movies")
```
...
ggplot() + 
    ...
    scale_x_log10(breaks = c(0.3e+08, 0.5e+08, 0.7e+08), 
                labels = c("30 million", "50 million", "70 million")) +
...
In contrast with the genres of the movies, the differences  in the average gross revenue based on the keywords is not that great. In particular, among the 20 most populat keywords *alien* seems to be the most profitable and *boy* the less profitable one with predicted gross revenues of US$ `r round(max(avg_gross$predicted_gross)/10^6, 2)` million and US$ `r round(min(avg_gross$predicted_gross)/10^6, 2)` million respectively. The difference between those two is US$ `r round((max(avg_gross$predicted_gross) -  min(avg_gross$predicted_gross))/10^6, 2)` million. Same as before, we will perform some T-tests between the average gross revenues of the movies with the most and least profitable plot keywords in order to find out if those differences are significant.

```{r echo=FALSE}
pander(t.test(movies_and_keywords_full$gross[movies_and_keywords_full$keywords == "alien"], movies_and_keywords_full$gross[movies_and_keywords_full$keywords == "boy"]), caption = "T-Test for Gross Revenue between alien and boy keywords")
```
...
ggplot(movieswithdirectordata, aes(x = dir_number_of_movies, y = gross)) + 
    ...
    geom_smooth() +
    theme_bw() +
    labs(x = "Number of movies per director", y = "Gross Revenue") +
    scale_y_log10(breaks = c(1, 1e+02, 1e+04, 1e+06, 1e+08, 1e+10), 
                  labels = c("1", "100", "10,000", "1m", "100 m", "10 bn"),
                  limits = c(1, 1e+10)) +
    annotation_logticks(sides = "lr", colour = "gray")
```
...
pander(cor.test(movieswithdirectordata$gross, movieswithdirectordata$dir_number_of_movies), caption=paste("Correlation Test for Gross Revenue and # of movies per director in sample, correlation= ",round(cor(movieswithdirectordata$gross, movieswithdirectordata$dir_number_of_movies),3))) 

##cor test for gross score vs # of movies for <10 movies
pander(cor.test(movieswithdirectordata$gross[movieswithdirectordata$dir_number_of_movies<10], movieswithdirectordata$dir_number_of_movies[movieswithdirectordata$dir_number_of_movies<10]), caption=paste("Correlation Test for Gross Revenue and # of movies per director in sample /n for directors with <10 movies, correlation= ",round(cor(movieswithdirectordata$gross[movieswithdirectordata$dir_number_of_movies<10], movieswithdirectordata$dir_number_of_movies[movieswithdirectordata$dir_number_of_movies<10]),3))) 
```
...
ggplot(movieswithdirectordata, aes(x = factor(more_than_ten_movies), y = gross, fill = factor(more_than_ten_movies))) + 
    ...
    labs(x = "",
         y = "Gross",
         title = "Gross revenue distribution by \n number of movies per director") +
    scale_y_log10(breaks = c(1, 1e+02, 1e+04, 1e+06, 1e+08, 1e+10), 
                  labels = c("1", "100", "10,000", "1m", "100 m", "10 bn"),
                  limits = c(1, 1e+10)) +
    annotation_logticks(sides = "lr", colour = "gray") +
...
pander(t, caption="T-test of means - Gross revenue for movies with directors with 10+ movies against those with fewer") 
```
...
pander(v, caption="F-test of variance - Gross revenue for movies with directors with 10+ movies against those with fewer") 
```
...
# Log Model
fullmodel_log <- lm(log(gross) ~ budget + num_voted_users + PG_13 + duration + cast_total_facebook_likes + director_facebook_likes + Animation + Family + Mystery + Drama + num_user_for_reviews,  data = trainwithdirectorandgenredata)

fullmodel2010_log <- lm(log(gross) ~ budget + num_voted_users + PG_13 + duration + cast_total_facebook_likes + director_facebook_likes + Animation + Family + Mystery + Drama + num_user_for_reviews,  data = trainwithdirectorandgenredata[trainwithdirectorandgenredata$title_year >=2010,])

stargazer(list(fullmodel_log, fullmodel2010_log), type = "html", align = TRUE, column.labels=c("For all years", "From 2010 onwards"),
          dep.var.labels = "Log(Gross revenue)", no.space = TRUE, title = "Linear Regression results (Log Model)")
```
</center>
...
##Log Model
englishmodel_log <- lm(log(gross) ~ budget + movie_facebook_likes + cast_total_facebook_likes + director_facebook_likes 
              + imdb_score + PG_13 + Action + Adventure + Animation + Documentary + Fantasy + Family 
              + us_or_others + english, 
              data = trainwithdirectorandgenredata)

englishmodel2010_log <- lm(log(gross) ~ budget + movie_facebook_likes + cast_total_facebook_likes + director_facebook_likes 
                  + imdb_score + PG_13 + Action + Adventure + Animation + Documentary + Fantasy + Family 
                  + us_or_others + english, 
                  data = trainwithdirectorandgenredata[trainwithdirectorandgenredata$title_year >= 2010, ])

stargazer(list(englishmodel_log, englishmodel2010_log), type = "html", align = TRUE, 
          column.labels=c("For all years", "From 2010 onwards"),
          dep.var.labels = "Log(Gross revenue)", no.space = TRUE, title = "Linear Regression results (Log Model)")
...
knownvariablesmodel_log <- lm(log(gross) ~ budget + PG_13 + duration + cast_total_facebook_likes + director_facebook_likes + Animation + Family + Mystery + Drama + more_than_ten_movies, data=trainwithdirectorandgenredata)

knownvariablesmodel2010_log <- lm(log(gross) ~ budget + PG_13 + duration + cast_total_facebook_likes + director_facebook_likes + Animation + Family + Mystery + Drama + more_than_ten_movies, data=trainwithdirectorandgenredata[trainwithdirectorandgenredata$title_year>=2010, ])

stargazer(list(knownvariablesmodel_log, knownvariablesmodel2010_log), type = "html", align = TRUE, column.labels=c("For all years", "From 2010 onwards"), 
          dep.var.labels = "Log(Gross revenue)", no.space = TRUE, title = "Linear Regression results (Log Model)")
```
</center>
...
## Log Model
##calculate the prediction and RMSE for all 3 models using all data and data post 2010
fullmodelpre_log <- exp(predict(fullmodel_log, test))
fullmodelrmse_log <- sqrt(mean((fullmodelpre_log - test$gross)^2))
englishmodelpre_log <- exp(predict(englishmodel_log, test))
englishmodelrmse_log <- sqrt(mean((englishmodelpre_log - test$gross)^2))
knownvariablesmodelpre_log <- exp(predict(knownvariablesmodel_log, test))
knownvariablesmodelrmse_log <- sqrt(mean((knownvariablesmodelpre_log - test$gross)^2))

fullmodel2010pre_log <- exp(predict(fullmodel2010_log, test[test$title_year >= 2010, ]))
fullmodel2010rmse_log <- sqrt(mean((fullmodel2010pre_log - test$gross[test$title_year >= 2010])^2))
englishmodel2010pre_log <- exp(predict(englishmodel2010_log, test[test$title_year >= 2010, ]))
englishmodel2010rmse_log <- sqrt(mean((englishmodel2010pre_log - test$gross[test$title_year >= 2010])^2))
knownvariablesmodel2010pre_log <- exp(predict(knownvariablesmodel2010_log, test[test$title_year >= 2010, ]))
knownvariablesmodel2010rmse_log <- sqrt(mean((knownvariablesmodel2010pre_log - test$gross[test$title_year >= 2010])^2))

##turn RMSE data into a table for display with columns of "all data" and "post 2010", and rows of model type
rmsetable_log <- data.frame(alldata=c(fullmodelrmse_log, englishmodelrmse_log, knownvariablesmodelrmse_log), post2010 = c(fullmodel2010rmse_log, englishmodel2010rmse_log, knownvariablesmodel2010rmse_log))
rownames(rmsetable_log) <- c("Full Model (Log)", "English Model(Log)", "Known Variables Model (Log)")
colnames(rmsetable_log) <- c("Model using all data", "Model using post 2010 data")
pander(rmsetable_log, format="markdown")
...
##Plot the predictions and residuals for full model (log) with all data
dffullmodel_log <- data.frame(y_actual = test$gross, y_predict = fullmodelpre_log)
plot1_log <- ggplot(data = dffullmodel_log, aes(x = y_predict, y = y_actual)) + geom_point(colour = "red") + 
    geom_abline(colour = "blue") + xlab("Predicted Gross Revenue (US$)") + 
    ylab("Actual Gross Revenue(US$)") + 
    coord_cartesian(xlim = c(-0.2e8, 2.5e8), ylim = c(0, 3e8)) + 
    scale_y_continuous(label = human_usd)  + scale_x_continuous(label = human_usd) + ggtitle("Full Model (Log) All Data")
plot2_log <- ggplot(data = dffullmodel_log, aes(x = y_predict, y = y_actual - y_predict)) + 
    geom_point(colour = "red") + geom_hline(yintercept = 0, colour = "blue") + 
    xlab("Predicted Gross Revenue (US$)") + ylab("Residual(US$)") + 
    coord_cartesian(xlim = c(-0.2e8, 2.5e8), ylim = c(-1.5e8, 2e8)) + 
    scale_y_continuous(label = human_usd) + scale_x_continuous(label = human_usd) + ggtitle("Full Model (Log) All Data")
##Plot the predictions and residuals for full model (log) post 2010
dffullmodel2010_log <- data.frame(y_actual = test$gross[test$title_year>=2010], y_predict = fullmodel2010pre_log)
plot3_log <- ggplot(data = dffullmodel2010_log, aes(x = y_predict, y = y_actual)) + geom_point(colour = "blue") + 
    geom_abline(colour = "red") + xlab("Predicted Gross Revenue (US$)") + 
    ylab("Actual Gross Revenue(US$)") + 
    coord_cartesian(xlim = c(-0.2e8, 2.5e8), ylim = c(0, 3e8)) + 
    scale_y_continuous(label = human_usd)  + scale_x_continuous(label = human_usd) + ggtitle("Full Model (Log) post 2010")
plot4_log <- ggplot(data = dffullmodel2010_log, aes(x = y_predict, y = y_actual - y_predict)) + 
    geom_point(colour = "blue") + geom_hline(yintercept = 0, colour = "red") + 
    xlab("Predicted Gross Revenue (US$)") + ylab("Residual(US$)") + 
    coord_cartesian(xlim = c(-0.2e8, 2.5e8), ylim = c(-1.5e8, 2e8)) + 
    scale_y_continuous(label = human_usd) + scale_x_continuous(label = human_usd) + ggtitle("Full Model (Log) post 2010")
grid.arrange(plot1_log, plot2_log, plot3_log, plot4_log, ncol=2)
...
##Plot the predictions and residuals for english model with all data
...
englishmodelplot2 <- ggplot(data = dfenglishmodel, aes(x = y_predict, y = y_actual - y_predict)) + 
    ...
    coord_cartesian(xlim = c(-0.2e8, 2.5e8), ylim = c(-1.5e8, 2e8)) + 
    scale_y_continuous(label = human_usd) + scale_x_continuous(label = human_usd) + ggtitle("English Model All Data")
##Plot the predictions and residuals for english model post 2010
...
##Plot the predictions and residuals for english model (log) with all data
dfenglishmodel_log <- data.frame(y_actual = test$gross, y_predict = englishmodelpre_log)
englishmodelplot1_log <- ggplot(data = dfenglishmodel_log, aes(x = y_predict, y = y_actual)) + geom_point(colour = "red") + 
    geom_abline(colour = "blue") + xlab("Predicted Gross Revenue (US$)") + 
    ylab("Actual Gross Revenue(US$)") + 
    coord_cartesian(xlim = c(-0.2e8, 2.5e8), ylim = c(0, 3e8)) + 
    scale_y_continuous(label = human_usd)  + scale_x_continuous(label = human_usd) + ggtitle("English Model (Log) All Data")
englishmodelplot2_log <- ggplot(data = dfenglishmodel_log, aes(x = y_predict, y = y_actual - y_predict)) + 
    geom_point(colour = "red") + geom_hline(yintercept = 0, colour = "blue") + 
    xlab("Predicted Gross Revenue (US$)") + ylab("Residual(US$)") + 
    coord_cartesian(xlim = c(-0.2e8, 2.5e8), ylim = c(-1.5e8, 2e8)) + 
    scale_y_continuous(label = human_usd) + scale_x_continuous(label = human_usd) + ggtitle("English Model (Log) All Data")
##Plot the predictions and residuals for english model (log) post 2010
dfenglishmodel2010_log <- data.frame(y_actual = test$gross[test$title_year>=2010], y_predict = englishmodel2010pre_log)
englishmodelplot3_log <- ggplot(data = dfenglishmodel2010_log, aes(x = y_predict, y = y_actual)) + 
    geom_point(colour = "blue") + 
    geom_abline(colour = "red") + xlab("Predicted Gross Revenue (US$)") + 
    ylab("Actual Gross Revenue(US$)") + 
    coord_cartesian(xlim = c(-0.2e8, 2.5e8), ylim = c(0, 3e8)) + 
    scale_y_continuous(label = human_usd)  + scale_x_continuous(label = human_usd) + ggtitle("English Model (Log) post 2010")
englishmodelplot4_log <- ggplot(data = dfenglishmodel2010_log, aes(x = y_predict, y = y_actual - y_predict)) + 
    geom_point(colour = "blue") + geom_hline(yintercept = 0, colour = "red") + 
    xlab("Predicted Gross Revenue (US$)") + ylab("Residual(US$)") + 
    coord_cartesian(xlim = c(-0.2e8, 2.5e8), ylim = c(-1.5e8, 2e8)) + 
    scale_y_continuous(label = human_usd) + scale_x_continuous(label = human_usd) + ggtitle("English Model (Log) post 2010")
grid.arrange(englishmodelplot1_log, englishmodelplot2_log, englishmodelplot3_log, englishmodelplot4_log, ncol=2)
```
...
##Plot the predictions and residuals for known variables model (log) with all data
dfknownvariablesmodel_log <- data.frame(y_actual = test$gross, y_predict = knownvariablesmodelpre_log)
knownvariablesmodelplot1_log <- ggplot(data = dfknownvariablesmodel_log, aes(x = y_predict, y = y_actual)) + geom_point(colour = "red") + 
    geom_abline(colour = "blue") + xlab("Predicted Gross Revenue (US$)") + 
    ylab("Actual Gross Revenue(US$)") + 
    coord_cartesian(xlim = c(-0.2e8, 2.5e8), ylim = c(0, 3e8)) + 
    scale_y_continuous(label = human_usd)  + scale_x_continuous(label = human_usd) + ggtitle("Known Variables Model (Log) All Data")
knownvariablesmodelplot2_log <- ggplot(data = dfknownvariablesmodel_log, aes(x = y_predict, y = y_actual - y_predict)) + 
    geom_point(colour = "red") + geom_hline(yintercept = 0, colour = "blue") + 
    xlab("Predicted Gross Revenue (US$)") + ylab("Residual(US$)") + 
    coord_cartesian(xlim = c(-0.2e8, 2.5e8), ylim = c(-1.5e8, 2e8)) + 
    scale_y_continuous(label = human_usd) + scale_x_continuous(label = human_usd) + ggtitle("Known Variables Model (Log) All Data")

##Plot the predictions and residuals for known variables model (log) post 2010
dfknownvariablesmodel2010_log <- data.frame(y_actual = test$gross[test$title_year>=2010], y_predict = knownvariablesmodel2010pre_log)
knownvariablesmodelplot3_log <- ggplot(data = dfknownvariablesmodel2010_log, aes(x = y_predict, y = y_actual)) + geom_point(colour = "blue") + 
    geom_abline(colour = "red") + xlab("Predicted Gross Revenue (US$)") + 
    ylab("Actual Gross Revenue(US$)") + 
    coord_cartesian(xlim = c(-0.2e8, 2.5e8), ylim = c(0, 3e8)) + 
    scale_y_continuous(label = human_usd)  + scale_x_continuous(label = human_usd) + ggtitle("Known Variables Model (Log) post 2010")
knownvariablesmodelplot4_log <- ggplot(data = dfknownvariablesmodel2010_log, aes(x = y_predict, y = y_actual - y_predict)) + 
    geom_point(colour = "blue") + geom_hline(yintercept = 0, colour = "red") + 
    xlab("Predicted Gross Revenue (US$)") + ylab("Residual(US$)") + 
    coord_cartesian(xlim = c(-0.2e8, 2.5e8), ylim = c(-1.5e8, 1.5e8)) + 
    scale_y_continuous(label = human_usd) + scale_x_continuous(label = human_usd) + ggtitle("Known Variables Model (Log) post 2010")
grid.arrange(knownvariablesmodelplot1_log, knownvariablesmodelplot2_log, knownvariablesmodelplot3_log, knownvariablesmodelplot4_log, ncol=2)
...
* In this dataset, English/US-made movies, have higher gross revenues than Non-English/Non-US-made movies, this is in line with the theory.
* R-rated movies appear to have a lower gross revenue than PG/PG-13 rated movies, again, this is in line with the expected theory.
* Although budget and revenue are correlated, this correlation is not particularly strong. Although this positive correlation is in line with the expected theory, the fact that it is not particularly strong shows that investors into movie venures take on a lot of risk, as there is only a low amount of variance that can be explained by budget alone.
...
* The genres that have the highest gross revenues are Animation, Adventure, Family, Fantasy and Action, and the genre with the lowest gross revenue is Documentary, which has a significantly lower revenue than the remaining genres, which are fairly well clustered together.
* The number of movies produced by a director has a positive impact on the gross revenue, in particular there is usually an increase in gross revenue if the movie has a director with more than 10 movies in the sample.
* The data available, with the models created in this report, only explain approximately 66% of the variance in gross revenue, and the model containing only factors known before the release of the model, explains only approximately 52% of the variance.

If the outcomes of this report were used to make a movie to maximise gross revenues, that movie would be a long, Animated, Family movie, made in the US in the English language, rated PG-13, casted by actors who are popular on facebook, made by a director who has a lot of facebook likes and has already made at least 9 movies. Although the plot keyword analysis would suggest that the movie was about aliens seeking revenge on an island, this may be seen as too derivative of the movie with the highest revenue in our sample, Avatar, and as such we would recommend a more original plot.
