...
* Our predictive models are able to predict the gross revenue with a Root-Mean-Square Error of 37-56 million, depending on the chosen variables in the particular model.
...
```{r echo=FALSE}
cortest1 <- cor.test(movieswithdirectordata$dir_number_of_movies, movieswithdirectordata$budget)
pander(cortest1, caption=paste("Pearson's Product-Moment Correlation test for number of movies per director against budget, correlation =",round(cor(movieswithdirectordata$dir_number_of_movies, movieswithdirectordata$budget),3)))
```

Even though this correlation was found to be statistically significant, with a p-value of `r cortest1$p.value`, it is quite small (`r round(cor(movieswithdirectordata$dir_number_of_movies, movieswithdirectordata$budget), 3)`), so analysis of movies by director sample on gross revenue can be continued without an obvious confounder of budget.
...
Also, the opposite of the initial intuition appears to be the case for variance, the movies made by directors with over 10 movies in the sample has higher variance (`r round(var(movieswithdirectordata$gross[movieswithdirectordata$more_than_ten_movies=="10+"]),3)`) than others, (`r round(var(movieswithdirectordata$gross[movieswithdirectordata$more_than_ten_movies=="Fewer than 10"]),3)`). This can be explained by the much larger number of movies made by directors with fewer than 10 movies.

```{r echo = FALSE,  message=FALSE, fig.width = 10, fig.align = "center"}
ttest1 <- t.test(movieswithdirectordata$gross[movieswithdirectordata$more_than_ten_movies=="10+"], movieswithdirectordata$gross[movieswithdirectordata$more_than_ten_movies=="Fewer than 10"], alternative="greater")

pander(ttest1, caption="T-test of means - Gross revenue for movies with directors with 10+ movies against those with fewer") 
```

```{r echo = FALSE,  message=FALSE, fig.width = 10, fig.align = "center"}
vartest1 <- var.test(movieswithdirectordata$gross[movieswithdirectordata$more_than_ten_movies=="10+"], movieswithdirectordata$gross[movieswithdirectordata$more_than_ten_movies=="Fewer than 10"])

pander(vartest1, caption="F-test of variance - Gross revenue for movies with directors with 10+ movies against those with fewer") 
```
