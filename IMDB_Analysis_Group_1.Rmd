...
* The aspect ratio column with many missing values is removed - as there are a high number of NAs, and this variable is not expected to be key to the analysis. 
...
Lastly, when we look at the scatterplots, we see that UK & US movies seem to show some symmetry about 'y = x' but the other countries have too few movies to determine whether there is any relationship between gross revenue and IMDB score.
...
The above plot shows the development of gross revenue since the release of Facebook in 2005. There is an upward trend compared to the plot with displaying years with more than ten entries.
...
We see that Documentaries are the less prfitable by far with an average estimated gross revenue of US$ `r round(min(avg_gross$predicted_gross)/10^6, 2)` millions, while the most profitable ones appear to be Animation movies, with a gross revenue of US$ `r round(max(avg_gross$predicted_gross)/10^6, 2)` millions, followed by Adventure, Family, Fantasy and Action movies. The difference between the highest and lowest average gross revenues based on genres is US$ `r round((max(avg_gross$predicted_gross) - min(avg_gross$predicted_gross))/10^6, 2)` millions. The rest genres appear to have more or less the same predicted gross revenue. To find out if those differences in the predicted  gross revenue values are significant, we will perform some T-tests between the average gross revenues of movies of different genres, which are actually corresponging to the predicted gross revenue values.
...
In contrast with the genres of the movies, the differences  in the average gross revenue based on the keywords is not that great. In particular, among the 20 most populat keywords *alien* seems to be the most profitable and *boy* the less profitable one with predicted gross revenues of US$ `r round(max(avg_gross$predicted_gross)/10^6, 2)` millions and US$ `r round(min(avg_gross$predicted_gross)/10^6, 2)` millions respectively. The difference between those two is US$ `r round((max(avg_gross$predicted_gross) -  min(avg_gross$predicted_gross))/10^6, 2)` millions. Same as before, we will perform some T-tests between the average gross revenues of the movies with the most and least profitable plot keywords in order to find out if those differences are significant.
